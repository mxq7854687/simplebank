postgres:
	docker run --name --network bank postgres12 -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=admin -d postgres:12-alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go samplebank/db/sqlc Store

.PHONY: postgres createdxb dropdb migrateup migratedown migrateup1 migratedown1 sqlc server mock